...
#include "../controller/bufferutil.h"
...
void VMouseControl::startVMouse(int port)
{
    ...
    connect(this, &VMouseControl::sendCtrMsg, vmouseCtrSocket, [vmouseCtrSocket](const QByteArray &buffer) {
        vmouseCtrSocket->write(buffer.data(),buffer.length());
...
void VMouseControl::sendMsg(VMouseState state,int xLocal,int yLocal,int xSize,int ySize)
{
    if (!m_workerThread.isRunning()) {
        return;
    }
    ...
    BufferUtil::write16(buffer, xLocal);
    BufferUtil::write16(buffer, yLocal);
    BufferUtil::write16(buffer, xSize);
    BufferUtil::write16(buffer, ySize);
    buffer.close();
    emit sendCtrMsg(byteArray);
}
